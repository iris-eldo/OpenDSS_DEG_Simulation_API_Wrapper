...
# Import the load modification function
from load_operations import modify_loads_in_neighborhood
# Import the new generator addition function
from generator_operations import add_generation_to_bus
...
def modify_load_endpoint():
    ...
    modify_loads_in_neighborhood(circuit, neighbourhood, factor) # Call external function
    circuit.solve_power_flow()
...
def add_generator_endpoint():
    ...
    add_generation_to_bus(circuit, bus_name, kw, phases) # Call external function
    circuit.solve_power_flow()
    current_details = get_current_state_details(circuit)
    save_state_to_file(current_details, "latest_api_results.txt")
...
def reset_simulation_endpoint():
    ...
    circuit = OpenDSSCircuit("") # Re-initialize the circuit
    circuit.solve_power_flow()
    current_details = get_current_state_details(circuit)
    save_state_to_file(current_details, "latest_api_results.txt")
